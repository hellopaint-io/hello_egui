...
/// Draw the image a previous pass left behind, without running or rendering the child.
///
/// `None` when there is nothing to reuse: no texture for this child yet, or one of the
/// wrong shape - a resize has to go through [`render`] again.
pub(crate) fn reuse(
    ...
    let size_in_pixels = size_in_pixels(size, pixels_per_point);
    ...
    let target = renderer
        ...
        .0
        .get(&id)
        .filter(|target| target.size_in_pixels == size_in_pixels && target.format == format)?;
...
/// The texture a child of this size ends up needing, in physical pixels.
fn size_in_pixels(size: Vec2, pixels_per_point: f32) -> [u32; 2] {
    [
...
pub(crate) fn render(ui: &Ui, render_state: &RenderState, request: Request) -> Option<Shape> {
    ...
    let size_in_pixels = size_in_pixels(padded_size, pixels_per_point);
